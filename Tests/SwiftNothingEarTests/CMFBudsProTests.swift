...
final class CMFBudsProTests: XCTestCase {

    func testBattery() {
        let model = DeviceModel.cmfBudsPro(.white)
        ...
        let batteryResponseBytes: [UInt8] = [
            ...
            0x03, 0x04, 0xC6, 0x02, 0x37, 0x03, 0x3C
        ]
        ...
        if case .budsWithCase(let caseLevel, let left, let right) = batteryResponse.parseBattery(model: model) {
            XCTAssertEqual(caseLevel.level, 70)
            XCTAssertEqual(left.level, 55)
    ...
    func testModelDetectionByNameAndSerial() {
        let cases: [(String, DeviceModel)] = [
            ("SH003001000000", .cmfBudsPro(.black)),
            ("SH003201000000", .cmfBudsPro(.white)),
            ("SH003401000000", .cmfBudsPro(.orange))
        ]

        for (serial, expected) in cases {
            XCTAssertEqual(DeviceModel.getModel(from: serial), expected)
            XCTAssertEqual(DeviceModel.getModel(for: "CMF Buds Pro", serialNumber: serial), expected)
        }
    }
}
