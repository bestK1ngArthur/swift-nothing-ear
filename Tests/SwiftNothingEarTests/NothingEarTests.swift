...
final class NothingEarTests: XCTestCase {
    ...
    func testModelDetectionByNameAndSerial() {
        let cases: [(String, DeviceModel)] = [
            ("SH006201000000", .ear(.white)),
            ("SH006101000000", .ear(.black))
        ]

        for (serial, expected) in cases {
            XCTAssertEqual(DeviceModel.getModel(from: serial), expected)
            XCTAssertEqual(DeviceModel.getModel(for: "Nothing Ear", serialNumber: serial), expected)
        }
    }

    func testDeviceConfigUsesBluetoothAddressWhenSerialIsEmpty() {
        let responseBytes: [UInt8] = [
            0x55, 0x60, 0x01, 0x06, 0x40, 0x57, 0x00, 0x02,
            0x09, 0x32, 0x2C, 0x31, 0x2C, 0x0A, 0x32, 0x2C,
            0x32, 0x2C, 0x31, 0x2E, 0x30, 0x2E, 0x31, 0x2E,
            0x35, 0x36, 0x0A, 0x32, 0x2C, 0x34, 0x2C, 0x0A,
            0x33, 0x2C, 0x31, 0x2C, 0x0A, 0x33, 0x2C, 0x32,
            0x2C, 0x31, 0x2E, 0x30, 0x2E, 0x31, 0x2E, 0x35,
            0x36, 0x0A, 0x33, 0x2C, 0x34, 0x2C, 0x0A, 0x34,
        ...
        guard let response = BluetoothResponse(data: responseBytes) else {
            XCTFail("Failed to parse captured device config response")
            return
        }
